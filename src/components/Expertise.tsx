...
/* ─ Sub-components ────────────────────────────────────────────────────── */
function SkillRow({ skill, accent, delay }: { skill: Skill; accent: AccentKey; delay: number; }) {
  ...
  return (
    <motion.div
      variants={{
        hidden: { opacity: 0 },
        visible: { opacity: 1, transition: { delay: rowDelay, duration: 0.01 } },
      }}
      initial="hidden"
      whileInView="visible"
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mb-1.5 min-w-0">
        <span className="text-xs sm:text-[13px] text-white/65 truncate">{skill.name}</span>
        <div className="flex items-center gap-2 shrink-0">
          {skill.proof && (
            <a href={skill.proof.href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 px-2.5 py-1.5 rounded-md bg-emerald-500/15 border border-emerald-500/30 hover:bg-emerald-500/25 transition-all max-w-[60px] sm:max-w-none">
              <svg viewBox="0 0 24 24" className="w-3 h-3 fill-current text-emerald-400 shrink-0" aria-hidden>
              ...
              <span className="text-[9px] sm:text-[10px] font-medium text-emerald-400 whitespace-nowrap">Proof</span>
            </a>
          )}
      ...
      <div className="h-[5px] bg-white/[0.06] rounded-full overflow-hidden">
        <motion.div
          ...
          variants={{
            hidden: { width: '0%' },
            visible: { width: `${skill.level}%`, transition: { duration: 1.1, ease: 'easeOut', delay: rowDelay } },
          }}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: '-5%' }}
...
export default function Expertise() {
  ...
  const activeContainerVariants = shouldReduce ? { hidden: {}, visible: {} } : sectionVariants;
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div variants={activeItemVariants} className="mb-12 text-center">
        ...
        {/* ── Single window card ── */}
        <motion.div variants={activeItemVariants} className={`rounded-2xl bg-[#0e0e14] border ${a.ring} overflow-hidden`}>
          {/* ── Window title bar ── */}
          <div className="flex items-center gap-2 px-3 sm:px-5 py-2 sm:py-3 bg-[#0a0a12] border-b border-white/[0.06]">
            ...
            <a
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[10px] sm:text-[11px] font-mono text-white/25 hover:text-violet-400 transition-colors"
            >
          ...
          <motion.div variants={activeItemVariants} className="flex border-b border-white/[0.06] overflow-x-auto scrollbar-none">
            {CATS.map(cat => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-3 sm:py-3.5 text-[10px] sm:text-xs font-mono font-medium whitespace-nowrap border-b-2 transition-all duration-200 min-w-[44px] ${
                    isActive
          ...
          <div className="p-3 sm:p-6 lg:p-8">
            <div className="flex flex-col lg:flex-row lg:items-start gap-6 lg:gap-8">
              ...
              {/* Left — skill bars */}
              <motion.div className="space-y-5" variants={activeContainerVariants}>
              ...
              <motion.div className="flex flex-col gap-5" variants={sectionVariants}>
                {/* Category header */}
                    ...
                    {active.skills
                      ...
                      .map(p => (
                        <motion.a
                          ...
                          className="flex items-center justify-between px-3 py-3 rounded-xl bg-white/[0.03] border border-white/[0.07] hover:border-violet-500/30 hover:bg-violet-500/5 transition-all duration-200 group"
                        >
                ...
                {/* Proof count */}
                <motion.div variants={activeItemVariants} className="mt-auto pt-4 border-t border-white/[0.06] flex items-center justify-between">
        ...
        {/* ── Currently learning ── */}
        <motion.div variants={activeItemVariants} className="mt-8 p-5 rounded-2xl border border-violet-500/15 bg-violet-500/[0.03] flex flex-wrap items-center gap-4">
          ...
          <div className="ml-auto flex gap-3">
            {[
              ...
              <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
                 className="px-3 py-2 rounded-md text-[11px] font-mono text-white/35 hover:text-violet-400 transition-colors">
                {l.label}
              </a>
            ))}
          </div>
