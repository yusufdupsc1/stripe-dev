  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-1.5 min-w-0">
        <span className="text-[13px] text-white/65 flex-1 min-w-0 truncate">{skill.name}</span>
        <div className="flex items-center gap-2 shrink-0">
          {skill.proof && (
            <a href={skill.proof.href} target="_blank" rel="noopener noreferrer" className="proof-badge">
          ...
          <span className={`text-xs font-mono tabular-nums shrink-0 ${a.label}`}>{skill.level}%</span>
        </div>
      </div>
      {/* Progress track */}
